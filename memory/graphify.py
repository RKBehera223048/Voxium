...
from memory.graph_clustering import (
    cluster_graph,
    score_all,
    label_communities,
    detect_bridge_nodes,
...
class MemoryGraph:
    ...
    # ── [M-2] Public lock-protected graph accessor ──────────────────────

    from contextlib import asynccontextmanager

    @asynccontextmanager
    async def locked_graph(self):
        """Yield the internal NetworkX graph under the asyncio lock.

        Use this instead of accessing ``_graph`` directly to prevent
        race conditions during concurrent ingest/recluster operations.

        Usage::

            async with memory_graph.locked_graph() as G:
    ...
    async def get_graph_data(
        ...
        """Return graph as node_link_data for API serialization.

        [M-1] Supports pagination to prevent memory exhaustion on large graphs.
        ...
        async with self._lock:
            ...
            # [M-1] Apply pagination if limit is specified
            all_nodes = data.get("nodes", [])
            if limit is not None:
                paginated_nodes = all_nodes[offset:offset + limit]
                paginated_node_ids = {n.get("id") for n in paginated_nodes}
                data["nodes"] = paginated_nodes
                # Only include edges between paginated nodes
                data["links"] = [
                    link for link in data.get("links", [])
                    if link.get("source") in paginated_node_ids
                    and link.get("target") in paginated_node_ids
                ]
            ...
            data["metadata"] = {
                ...
                "offset": offset,
                "limit": limit,
                "returned_nodes": len(data.get("nodes", [])),
            }
            return data
